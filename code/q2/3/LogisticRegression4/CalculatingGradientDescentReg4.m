function[theta_mat,j_history] = CalculatingGradientDescentReg4(mat,y,theta,alpha,iterations)
m = length(y);
j_history = zeros(iterations, 1);
theta_mat = ones(iterations, 5);
for i =1:iterations
    h = 1./(1 + exp(-(mat* theta)));
    ...
    t5 = theta(5) - alpha*(1/m)*sum((h-y).*mat(:, 5));
    theta(1) = t1;
    theta(2) = t2;
    theta(3) = t3;
    theta(4) = t4;
    ...
    theta_mat(i,5) = theta(5);
    j_history(i) = CalculatingCostReg4(mat,y,theta);
end
end